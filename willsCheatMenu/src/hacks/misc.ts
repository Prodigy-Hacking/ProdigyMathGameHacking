...
new Hack(category.misc, "Restore Player State", "Restores your player data to an earlier state (WCM must've been installed at the time, and the history data is local to this device only.)").setClick(async () => {
  ...
  inputOptionsPromise = new Promise(function(resolve) {
    setTimeout(function() {
      resolve({
        for (let i = 0; i < playerStateHistory.length; i++) {
	        arr = [];
	        newItem = Object.keys(playerStateHistory)[i];
	        if(newItem == 0) {
		        newItem = "0 - Oldest item";
  	      } else if(newItem == playerStateHistory.length) {
		        newItem = `${playerStateHistory.length} - Latest item`;
          }
          arr.push(newItem);
	        return arr;
        }
  ...
  const { value: stateID } = await Swal.fire({
      title: 'Choose a state to restore from',
      input: 'select',
      inputOptions: inputOptionsPromise
    },
    inputPlaceholder: 'Select a player state ID. The larger the number, the more recent the saved state is, and vice versa.',
